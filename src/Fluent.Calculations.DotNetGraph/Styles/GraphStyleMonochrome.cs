...
namespace Fluent.Calculations.DotNetGraph.Styles
{
    internal class GraphStyleMonochrome : IGraphStyle
    {
        public DotSubgraph CreateParametersCluster(string scope, int index)
        {
            ...
            subgraph.SetPenWidth(1);
            subgraph.SetFontToCourierNew();
    ...
    internal static class GraphStyleMonochromeExtensions
    {
        private static string CourierNewFontName = "Courier New";

        public static void SetFontToCourierNew<T>(this T node) where T : DotElement => node.SetFontName(CourierNewFontName);

        public static void SetFontName<T>(this T node, string fontName) where T : DotElement => node.SetAttribute("fontname", new DotAttribute(@$"""{fontName}"""));

        public static void SetPenWidth<T>(this T node, int penwidth) where T : DotElement => node.SetAttribute("penwidth", new DotAttribute(@$"""{penwidth}"""));
    }
